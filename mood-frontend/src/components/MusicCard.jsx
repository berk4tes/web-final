// MusicCard — Spotify/Apple-Music-inspired playlist row with cover, title, artist, save + open buttons
...
const MusicCard = ({ item, isFavorite, onToggleFavorite }) => {
  ...
  const spotifyQuery = encodeURIComponent(`${item.title} ${artist}`.trim());
  ...
  return (
    <article className="group flex items-center gap-4 rounded-2xl border border-ink-100 bg-white p-3 transition hover:border-accent/40 hover:bg-ink-50/40 hover:shadow-soft">
      <div className="relative h-16 w-16 flex-shrink-0 overflow-hidden rounded-xl">
        <img
          ...
          loading="lazy"
          onError={(e) => {
            e.currentTarget.src = MUSIC_PLACEHOLDER;
          }}
        />
      </div>
      ...
      <div className="min-w-0 flex-1">
        ...
        {artist && <p className="truncate text-xs text-ink-400">{artist}</p>}
        {item.aiExplanation && (
