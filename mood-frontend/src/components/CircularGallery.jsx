...
class App {
  constructor(
    ...
    this.originalLength = items?.length || 0;
    this.activeIndex = -1;
  ...
  updateActiveIndex() {
    if (!this.originalLength || !this.medias?.[0]) return;
    const width = this.medias[0].width;
    const nextIndex = mod(Math.round(this.scroll.current / width), this.originalLength);
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    const app = new App(containerRef.current, {
      ...
      scrollSpeed,
      scrollEase,
      onActiveIndexChange: (index) => activeChangeRef.current?.(index),
    });
